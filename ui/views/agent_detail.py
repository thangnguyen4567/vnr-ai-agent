...
def render_agent_detail():
    st.title("🧩 Multi-Agent - Cấu hình chi tiết")
    configs_dir = "settings/multi_agents"
    selected_file = st.session_state.get("selected_multi_agent_file", "")
    if not selected_file:
        st.warning("Chưa chọn cấu hình. Quay lại danh sách để chọn.")
        if st.button("⬅️ Quay lại danh sách", key="detail_back_no_selection"):
            st.session_state.agent_config_view = "overview"
            st.rerun()
        return
    yaml_config_path = f"{configs_dir}/{selected_file}"
    st.caption(f"File: {yaml_config_path}")
    if st.button("⬅️ Quay lại danh sách", key="detail_back"):
        st.session_state.agent_config_view = "overview"
        st.rerun()
